  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        setErrorMessage(emailErrors[0] ?? passwordErrors[0] ?? result.error.message);
        return;
      }
      ...
      setErrorMessage("No pudimos iniciar sesion. Intenta de nuevo.");
    } finally {
  ...
  return (
    <section className="py-16 sm:py-20">
      <div className="mx-auto w-full max-w-md rounded-3xl border border-white/10 bg-[#111420]/90 p-6 shadow-[0_18px_50px_rgba(0,0,0,0.35)] sm:p-8">
        <h1 className="text-2xl font-semibold text-[#F5F7FA]">Iniciar sesion</h1>
        <p className="mt-2 text-sm text-[#98A0B3]">
          Entra con tu correo y contrasena para ir al dashboard.
        </p>
        ...
        <p className="mt-5 text-sm text-[#98A0B3]">
          ...
          <Link href="/signup" className="text-[#F5F7FA] underline decoration-[#E625FF]/60 underline-offset-4">
            Crear cuenta
          </Link>
        </p>
      </div>
